...
AZURE_OPENAI_EMBEDDING_BATCH_SIZE = int(
	os.getenv("AZURE_OPENAI_EMBEDDING_BATCH_SIZE", "32")
)
...
AZURE_COGNITIVE_SEARCH_SEMANTIC_CONFIG_NAME = os.getenv(
	"AZURE_COGNITIVE_SEARCH_SEMANTIC_CONFIG_NAME", "latimer-semantic-config"
)
AZURE_COGNITIVE_SEARCH_SEMANTIC_ENABLED = os.getenv(
	"AZURE_COGNITIVE_SEARCH_SEMANTIC_ENABLED", "true"
).strip().lower() not in {"0", "false", "no", "off"}
AZURE_COGNITIVE_SEARCH_VECTOR_CANDIDATES = int(
	os.getenv("AZURE_COGNITIVE_SEARCH_VECTOR_CANDIDATES", "50")
)
...
ISSP_PROCESSED_PATH = os.getenv(
	"ISSP_PROCESSED_PATH", "data/issp/issp_questions_tagged.json"
)
